...
* Attention		 : None
*******************************************************************************/
void LCD_DisplayStringLine(uint16_t Xpos, uint16_t Ypos, uint8_t *str, uint16_t Color, uint16_t bkColor)
{
    uint8_t TempChar;
    do
    {
...
/******************************************************************************
* Function Name  : GUI_Text_variables
* Description    : 
* Input          : - Xpos: 
*                  - Ypos: 
*				           - str:
...
{	
	...
	sprintf(lcd_buffer,"FPS:%d",str);
	LCD_DisplayStringLine( Xpos, Ypos, lcd_buffer, Color, bkColor );
}
...
void LCD_WindowModeDisable(void)
{
  ...
  LCD_WriteReg(0x0003, 0x1018);    // Entry mode
}
...
void LCD_SetAddressDisplay(uint16_t Xpos, uint16_t Ypos, uint16_t dX, uint16_t dY)
{
	uint16_t hor = 0, vert = 0;
	hor = Xpos + dX - 1;
	vert = Ypos + dY - 1;
	LCD_WriteReg(0x0050,Xpos);	// Horizontal address start position
	LCD_WriteReg(0x0051,hor);	// Horizontal address end position
	LCD_WriteReg(0x0052,Ypos);	// Vertical address start position
	LCD_WriteReg(0x0053,vert);	// Vertical address end position
